...
class RealtimeDialogClient:
    def __init__(self, config: Dict[str, Any], session_id: str, output_audio_format: str = "pcm",
                 mod: str = "audio", recv_timeout: int = 10, start_session_req: Dict[str, Any] = None) -> None:
        self.config = config
        self.logid = ""
        ...
        # 使用传入的 start_session_req 或全局配置的深拷贝，避免修改原始对象
        if start_session_req is not None:
            self.start_session_req = copy.deepcopy(start_session_req)
        else:
            self.start_session_req = copy.deepcopy(config.start_session_req)
        self.ws = None
    ...
    async def connect(self) -> None:
        ...
        if self.output_audio_format == "pcm_s16le":
            # 确保 audio_config 字典存在
            if "audio_config" not in self.start_session_req["tts"]:
                self.start_session_req["tts"]["audio_config"] = {}
